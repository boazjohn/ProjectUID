...
mysql_query("UPDATE udetails_ext_bill SET active = '$val_0'
WHERE uid_id = '$uidd' AND id = '$id'");

mysql_query("UPDATE udetails_ext_bank SET bank_amount = '$amount'
WHERE uid_id = '$uidd' AND bankaccount = '$bnkaccnt'");
$data='Bill paid of Rs.';$data.=$amt;$data.='/- Bill Details: ';$data.=$desc;$data.=' (';$data.=$type;$data.='), Dated: ';$data.=$date;$data.=' from Account, ';$data.=$bnkaccnt;$data.='. Balance amount: Rs.';$data.=$amount;$data.='/-';		
